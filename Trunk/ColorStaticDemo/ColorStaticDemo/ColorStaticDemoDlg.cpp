...
CColorStaticDemoDlg::CColorStaticDemoDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_COLORSTATICDEMO_DIALOG, pParent)
{
...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_STATIC_DEMO, m_ColorText);
}

BEGIN_MESSAGE_MAP(CColorStaticDemoDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
END_MESSAGE_MAP()
...
BOOL CColorStaticDemoDlg::OnInitDialog()
{
	...
	m_ColorText.SetFont(200, _T("ËÎÌו"));
	//m_ColorText.SetBackColor(RGB(255, 0, 0));
	m_ColorText.SetTextColor(RGB(255, 0, 0));
