// XTHexEdit.cpp : implementation of the CHexEdit class.
//
...
int SPRINTF_S(TCHAR *buffer, size_t count, const TCHAR *format, ...) {
	va_list args;
...
void MEMMOVE_S(void *dest, const void *src, size_t count) {
	#if (_MSC_VER > 1310) // VS2005
...
///
IMPLEMENT_DYNAMIC(CHexMemDC, CDC)

//===========================================================================
// CHexMemDC class
//===========================================================================

CHexMemDC::CHexMemDC(CDC* pDC, const CRect& rect, COLORREF clrColor/*=GetSysColor(COLOR_3DFACE)*/)
{
	ASSERT(pDC != NULL);
	m_pDC = pDC;
...
CBitmap& CHexMemDC::GetBitmap()
{
	return m_bitmap;
}

void CHexMemDC::Discard()
{
	m_bValid = FALSE;
}

void CHexMemDC::FromDC()
{
...
CHexEdit::CHexEdit()
{
	...
	m_crDisabledBack    = GetSysColor(COLOR_3DFACE);
	m_crDisabledText    = GetSysColor(COLOR_GRAYTEXT);
	m_crHighlightBack   = GetSysColor(COLOR_HIGHLIGHT);
	m_crHighlightText   = GetSysColor(COLOR_HIGHLIGHTTEXT);
	m_crDisabledHighlightText = GetSysColor(COLOR_HIGHLIGHTTEXT);
	m_crDisabledHighlightBack = GetSysColor(COLOR_3DSHADOW);
...
void CHexEdit::RestoreDefaultFont()
{
	if (m_fontHex.GetSafeHandle())
		m_fontHex.DeleteObject();
...
void CHexEdit::SetHexFont(CFont* pFont)
{
...
void CHexEdit::SetHexFont(LOGFONT* pLogFont)
{
...
COLORREF CHexEdit::GetActualBackColor()
{
...
COLORREF CHexEdit::GetActualTextColor()
{
	if ( !IsWindowEnabled( ) )
	{
...
COLORREF CHexEdit::GetActualHighlightBackColor()
{
	if ( !IsWindowEnabled( ) )
	{
...
COLORREF CHexEdit::GetActualHighlightTextColor()
{
	if ( !IsWindowEnabled( ) )
	{
...
void CHexEdit::OnPaint()
{
	...
	CHexMemDC memDC( &dc, rectClient, GetActualBackColor( ) );
...
void CHexEdit::DoVScroll(UINT nSBCode, bool bMoveCaret)
{
	if(!m_pData)
	{
		return;
...
BOOL CHexEdit::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	...
	if(nChar == 0x08) // backspace
	{
		if (GetAllowDeletes())
		{
			...
			else if(m_nCurrentAddress > 0)
			{
				...
				RepositionCaret(m_nCurrentAddress);
				RedrawWindow();
				GetParent()->SendMessage(WM_HEXEDIT_CHANGE,0,0);
			}
			else
			{
				MessageBeep(0);
...
void CHexEdit::SetSel(int nSelStart, int nSelEnd)
{
#ifdef _DEBUG
	_CrtCheckMemory();
#endif
...
void CHexEdit::SelDelete(int nSelStart, int nSelEnd)
{
	if (!GetAllowDeletes())
		return;
...
void CHexEdit::SelInsert(int nSelStart, int nLength)
{
...
void CHexEdit::SetData(LPBYTE p, int nLength, int nMaxLength)
{
	if(m_pData)
		free(m_pData);
...
DWORD CHexEdit::SetAddressBase(DWORD dwBase)
{
...
void CHexEdit::EnableScrollBars(bool /*bVertical*/, bool /*bHorizontal*/)
{
/*
	XT_ASSERT_MSG(FALSE,
		"CHexEdit::EnableScrollBars has been deprecated.\n"
		"\n"
...
int CHexEdit::GetData(LPBYTE pData, int nLength)
{
